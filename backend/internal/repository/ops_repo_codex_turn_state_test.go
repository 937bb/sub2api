...
func TestLoadPreferredOpenAICodexTurnStateUsesBucketAndConfiguredOrder(t *testing.T) {
	...
	query := `(?s)SELECT id, state_value.*WHERE source_account_id = \$1 AND source_model = \$2.*value_length = ANY\(\$3::integer\[\]\) AND expires_at > \$4.*issued_at <= \$4 AND issued_at \+ INTERVAL '4 minutes' > \$4.*source_transport = 'scanner' AND source_session_id IS NOT NULL.*ORDER BY array_position\(\$3::integer\[\], value_length\).*LIMIT 1`
