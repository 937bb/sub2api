...
func TestForwardAsAnthropic_OAuthRestoresCodexIdentityHeaders(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			require.NoError(t, err)
			require.NotNil(t, result)
			requireOpenAIMessagesCodexIdentity(t, upstream.lastReq, resolveCodexOutboundIdentity(codexAccountUserAgent(account)).userAgent, openai.CodexDefaultOriginator)
		})
	}
}
