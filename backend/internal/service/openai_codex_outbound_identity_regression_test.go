	...
	for _, accountType := range []string{AccountTypeOAuth, AccountTypeSetupToken} {
		for _, sessionHeader := range []string{"session-id", "session_id", ""} {
			t.Run(accountType+"/"+sessionHeader, func(t *testing.T) {
				...
				require.NoError(t, err)
				applyCodexNormalizedRequestIdentityHeaders(c, account, regular.Header, normalized)
				normalizeCodexResponsesTransportHeaders(regular, account)
				applyCodexNormalizedRequestIdentityHeaders(c, account, passthrough.Header, normalized)
				normalizeCodexResponsesTransportHeaders(passthrough, account)
				applyCodexNormalizedRequestIdentityHeaders(c, account, ws, normalized)
...
func TestCodexCacheOnlyIdentityIsNotHashedTwice(t *testing.T) {
	...
	require.Equal(t, scopeCodexAccountIdentityValue(account, 91, "prompt-cache", "conversation-a"), gjson.GetBytes(body, "prompt_cache_key").String())
