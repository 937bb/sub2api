...
type SystemSettings struct {
	...
	EnableClaudeOAuthSystemPromptInjection bool   // 是否注入 Claude OAuth system prompt blocks（默认 true）
	ClaudeOAuthSystemPrompt                string // 自定义 Claude OAuth system prompt；空值使用内置 Claude Code prompt
	ClaudeOAuthSystemPromptBlocks          string // 自定义 Claude OAuth system blocks JSON；空值使用内置 3-block 模板
