...
type SystemSettings struct {
	...
	OpenAITTFTMode                           string // Responses first_token_ms measurement mode
	...
	OpenAIOAuthWSDefaultEnabled              bool   // OAuth/PAT/Setup Token 未配置账号模式时默认使用 WS（默认 true）
	OpenAICodexFingerprintDefaultFullEnabled bool   // 未配置账号指纹模式时默认 full（默认 true）
