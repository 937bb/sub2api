...
type OpenAIImagesUpstreamError struct {
	...
	// SynthesizedFromModelText marks an error inferred from plain model text
	// rather than a structured upstream error frame. It is retryable for this
	// turn, but must not cool the account-level image capability.
	SynthesizedFromModelText bool
}

// openAIImagesSelfBuiltRequestContextKey marks image requests whose body is
// constructed by the gateway, so capability-loss handling can distinguish them
// from client-controlled Responses payloads.
...
func openAIImagesTextFallbackErrorForText(text string) *OpenAIImagesUpstreamError {
	...
	return &OpenAIImagesUpstreamError{
		...
		// A plain-text answer is prompt-dependent and does not prove that the
		// account's image tool is unavailable for subsequent requests.
