...
type UpstreamFailoverError struct {
	...
	SameAccountRetryLimit     int         // 请求级重试上限覆盖；0 表示继续使用账号的 pool-mode 配置
	ClearRateLimitBeforeRetry bool        // 仅在确定执行下一次同账号尝试时清除刚写入的 429 状态
	RateLimitObservedBefore   time.Time   // 仅清除不晚于本次 429 的持久化状态，避免覆盖其他并发写入的新一代限流
	RuntimeBlockGeneration    uint64      // 本次 429 完成时观察到的内存阻断版本
...
// ResolveSameAccountRetryLimit applies a request-scoped override without
// changing the account's regular pool-mode retry configuration.
func (e *UpstreamFailoverError) ResolveSameAccountRetryLimit(fallback int) int {
