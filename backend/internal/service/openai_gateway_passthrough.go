...
func (s *OpenAIGatewayService) forwardOpenAIPassthrough(
	...
	if account != nil && account.UsesOpenAICodexProtocol() {
		...
		if !isOpenAIResponsesCompactPath(c) {
			fpIDs := s.resolveCodexIsolatedFingerprintForRequest(ctx, c, codexAccountIdentitySource(c, account), body)
			if fpIDs != nil {
	...
	forcedBody, forceErr := forceOpenAIPriorityTierInBody(apiKey, account, body)
	if forceErr != nil {
		return nil, forceErr
	}
	body = forcedBody
...
func (s *OpenAIGatewayService) buildUpstreamRequestOpenAIPassthrough(
	...
	s.guardOpenAICodexTurnStateEcho(c, account, req.Header, extractOpenAICodexTicketModel(body))
...
func openAIStreamClientOutputStarted(c *gin.Context, localStarted bool) bool {
	...
	return OpenAIStreamAdjustedWrittenSize(c) >= 0
}
...
func isOpenAIUpstreamCapacityShedCode(payload []byte) bool {
...
func isOpenAIUpstreamCapacityShedEvent(payload []byte) bool {
	if isOpenAIUpstreamCapacityShedCode(payload) {
		return true
	}
...
func openAIStreamFailureStatus(payload []byte, message string) int {
	...
	// Keep generic failed events on the existing 502 failover path. Capacity
	// promotion is request-scoped and controlled by the group's retry switch,
	// while credential and rate-limit statuses retain their typed semantics.
	switch semanticStatus := openAIStreamFailedEventSemanticStatus(payload, message); semanticStatus {
...
func (s *OpenAIGatewayService) newOpenAIStreamFailoverError(
	...
	statusCode := s.openAIStreamFailureStatusForRequest(c, payload, message)
	_, shouldDisable := s.handleOpenAIStreamTerminalAccountSideEffects(c, account, payload, message, headers)
...
func (s *OpenAIGatewayService) handleStreamingResponsePassthrough(
	...
	events := make(chan scanEvent, openAIFirstOutputEventQueueSize(false))
...
streamLoop:
	for {
		...
		case <-intervalCh:
			...
			if !openAIStreamClientOutputStarted(c, clientOutputStarted) {
				return resultWithUsage(), s.newOpenAIStreamFailoverError(
					c, account, true, upstreamRequestID, nil, "OpenAI stream data interval timeout",
				)
			}
		...
		case <-keepaliveCh:
			if clientDisconnected || eventInProgress || time.Since(lastDownstreamWriteAt) < keepaliveInterval {
				continue
			}
		...
		if data, ok := extractOpenAISSEDataLine(line); ok {
			...
			if trimmedData == "[DONE]" && !sawTerminalEvent {
				dataBytes = buildOpenAIPassthroughCompletedEvent(responseID, usage)
				trimmedData = string(dataBytes)
				eventType = "response.completed"
				line = "data: " + trimmedData
				normalizedDoneTerminal = true
			}
			...
			if (eventType == "response.completed" || eventType == "response.done") &&
				!normalizedDoneTerminal && !sawFailedEvent && !semanticOutputSeen && !clientOutputStarted &&
