...
func (s *OpenAIGatewayService) ForwardAlphaSearch(ctx context.Context, c *gin.Context, account *Account, body []byte) (*OpenAIForwardResult, error) {
	...
	upstreamModel := resolveOpenAIModelForUpstream(c, account, account.GetMappedModel(requestedModel))
	...
	if resp.StatusCode >= http.StatusBadRequest {
		...
		if s.shouldFailoverOpenAIUpstreamResponse(resp.StatusCode, upstreamMessage, respBody) ||
			...
			shouldDisable := false
			if shouldApplyOpenAIAlphaSearchAccountErrorSideEffects(resp.StatusCode) && !isOpenAIUpstreamCapacityShedEvent(respBody) {
