...
func (s *OpenAIGatewayService) forwardAsRawChatCompletions(
	...
	forcedBody, forceErr := forceOpenAIPriorityTierInBody(getAPIKeyFromContext(c), account, upstreamBody)
	if forceErr != nil {
		return nil, forceErr
	}
	...
	// Use the final outbound body after policy filtering as the billing fallback.
