...
// codexFingerprintDerivationSeed prefers the official random per-account
// seed. Accounts created before the seed migration retain the secondary
// account-ID derivation so the branch's default-full behavior does not turn
// off during a rolling upgrade or for setup-token account snapshots.
func codexFingerprintDerivationSeed(account *Account) (string, bool) {
	...
	// Explicitly enabled modes use the official managed-seed contract. A
	// missing or malformed seed must fail closed; repository create/update
	// paths generate it atomically. The legacy fallback is only for accounts
	// inheriting this branch's default-full setting.
	switch codexFingerprintMode(strings.TrimSpace(account.GetExtraString(codexFingerprintModeExtraKey))) {
	case codexFingerprintDevice, codexFingerprintSession, codexFingerprintFull:
		return "", false
	}
	if account.ID <= 0 {
	...
	return fmt.Sprintf("legacy-account-id:%d", account.ID), true
}
