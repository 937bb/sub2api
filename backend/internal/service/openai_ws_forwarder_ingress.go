...
func (s *OpenAIGatewayService) ProxyResponsesWebSocketFromClient(
	...
	if modeRouterV2Enabled && !forceHTTPBridge {
		ingressMode = resolveOpenAIWSRoutingMode(account, s.cfg.Gateway.OpenAIWS.IngressModeDefault)
	...
	parseClientPayload := func(turn int, raw []byte) (openAIWSClientPayload, error) {
		...
		forcedFrame, forceErr := forceOpenAIPriorityTierInBody(getAPIKeyFromContext(c), account, normalized)
		if forceErr != nil {
		...
		for {
			...
			if eventType == "error" {
				...
				if !wroteDownstream && isOpenAIWSRateLimitError(errCodeRaw, errTypeRaw, errMsgRaw) {
					lease.MarkBroken()
					failoverErr := s.newOpenAIWSRateLimitFailoverError(account, lease.HandshakeHeaders(), upstreamMessage, errMsgRaw)
					return nil, s.configureOpenAIQuotaBypass429RetryEnabled(account, failoverErr, turnQuotaBypassEffective, false)
				}
			}
	...
	for {
		...
		upstreamPayload := applyOpenAIWSQuotaBypass(currentPayload, hooks)
		result, relayErr := sendAndRelay(turn, sessionLease, upstreamPayload, len(upstreamPayload), currentOriginalModel, currentImageBillingModel, currentImageSizeTier, currentImageInputSize, currentRequestedReasoningEffort)
		if relayErr != nil {
			lastTurnClean = false
			if isOpenAIWSSessionPreempted(ctx) {
				sessionLease.MarkBroken()
