...
func TestOpenAICodexTurnStatePool_ReobservingOlderStateKeepsNewerExpiry(t *testing.T) {
	for _, length := range []int{292, 332} {
		t.Run(strconv.Itoa(length), func(t *testing.T) {
			...
			pool.observe(older, &accountID, "old-session", model, "http")
			now = now.Add(time.Minute)
			pool.observe(newer, &accountID, "new-session", model, "http")
			pool.observe(otherAccount, &otherAccountID, "other-account", model, "http")
			pool.observe(otherModelState, &accountID, "other-model", otherModel, "http")
			now = now.Add(time.Minute)
			pool.observe(older, &accountID, "old-session-repeated", model, "http")
...
func TestOpenAICodexTurnStatePool_RefreshUsesHealthyFallbackWithinBucket(t *testing.T) {
	...
	pool.observe(preferred, &accountID, "preferred", model, "http")
	require.False(t, pool.hasReusableStateBeyond(accountID, model, deadline))
	pool.observe(fallback, &accountID, "fallback", model, "http")
