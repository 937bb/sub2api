...
const (
	...
	// Official codex-tui uses app-server clientInfo to produce:
	// {originator}/{engine_version} ({OS} {OS_version}; {arch}) {terminal} ({client_name}; {client_version})
	// The engine and client versions are the same release for the bundled TUI.
	codexCLIUserAgentEnvironment = " (Ubuntu 24.04; x86_64) xterm-256color"
	// codexCLIUserAgent 是编译期兜底 UA；运行时优先使用由后台版本号拼出的规范 UA。
	// 版本段必须来自 codexCLIVersion：UA 与 version 头是同一个版本声明的两个出口，
	// 各自硬编码会漂移成互相矛盾的身份。
	codexCLIUserAgent = openai.CodexDefaultOriginator + "/" + codexCLIVersion + codexCLIUserAgentEnvironment + " (" + openai.CodexDefaultOriginator + "; " + codexCLIVersion + ")"
...
type OpenAIGatewayService struct {
	...
	openaiQuotaBypassConcentratorOnce sync.Once
	...
	openaiQuotaBypassConcentrator     *openAIQuotaBypassConcentrator
...
func (s *OpenAIGatewayService) logOpenAIWSModeBootstrap() {
	...
	logOpenAIWSModeInfo(
		...
		ResolveOpenAIWSUpstreamReadLimitBytes(s.cfg),
		wsCfg.ClientReadLimitBytes,
	)
}
