...
func TestBuildBillingAttributionBlockJSON_NoCCH(t *testing.T) {
	body := []byte(`{"messages":[{"role":"user","content":[{"type":"text","text":"hello from cli"}]}]}`)
	block, err := buildBillingAttributionBlockJSON(body, "2.1.162")
	require.NoError(t, err)

	text := gjson.GetBytes(block, "text").String()
	require.Contains(t, text, "x-anthropic-billing-header:")
	require.Contains(t, text, "cc_version=2.1.162.")
	require.Contains(t, text, "cc_entrypoint=cli")
	require.NotContains(t, text, "cch=")
}
