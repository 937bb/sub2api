...
func (s *OpenAIGatewayService) proxyResponsesWebSocketV2Passthrough(
	...
	// A passthrough call can be retried with another account on the same
	// request context. Do not reuse the previous account's staged IDs.
	...
	if hooks != nil && hooks.ResolveRequestModel != nil {
		mappedModel, _, mapErr := hooks.ResolveRequestModel(1, initialRequestModel)
		if mapErr != nil {
			return mapErr
		}
	...
	forcedFirst, forceErr := forceOpenAIPriorityTierInBody(apiKey, account, firstClientMessage)
	if forceErr != nil {
		return NewOpenAIWSClientCloseError(coderws.StatusPolicyViolation, "invalid websocket request payload", forceErr)
	}
	...
	fingerprintedFirst, fingerprintErr := s.applyCodexFingerprintToWebSocketPayload(ctx, c, account, firstClientMessage)
	...
	for {
		...
		if statusCode == http.StatusTooManyRequests {
			...
			failoverErr := s.newOpenAIWSRateLimitFailoverError(account, handshakeHeaders, nil, err.Error())
			return s.configureOpenAIQuotaBypass429RetryEnabled(account, failoverErr, false, true)
		}
	...
	policyClientConn := &openAIWSPolicyEnforcingFrameConn{
		...
		filter: func(msgType coderws.MessageType, payload []byte) (out []byte, blocked *OpenAIFastBlockedError, filterErr error) {
			...
			if isResponseCreate {
				...
				if hooks != nil && hooks.ResolveRequestModel != nil {
					upstreamModel, _, err := hooks.ResolveRequestModel(turnNo, requestModelForThisFrame)
					if err != nil {
						return payload, nil, err
					}
			...
			if isResponseCreate {
				forcedPayload, forceErr := forceOpenAIPriorityTierInBody(apiKey, account, payload)
				if forceErr != nil {
	...
	relayResult, relayExit := openaiwsv2.RunEntry(openaiwsv2.EntryInput{
		...
		Options: openaiwsv2.RelayOptions{
			...
			BeforeWriteClient: func(msgType coderws.MessageType, payload []byte, wroteDownstream bool) error {
				...
				return s.configureOpenAIQuotaBypass429RetryEnabled(account, failoverErr, quotaBypassEffective.Load(), true)
			},
