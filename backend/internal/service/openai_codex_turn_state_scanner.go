...
func (s *openAICodexTurnStateScanner) enqueueSweep(ctx context.Context) {
	accounts, err := s.accountRepo.ListAllWithFilters(ctx, PlatformOpenAI, "", StatusActive, "", 0, "")
	if err != nil {
	...
	for i := range accounts {
		account := &accounts[i]
		if account.ParentAccountID != nil || !account.IsOpenAIOAuthLike() {
			continue
		}
		models := []string{defaultOpenAICodexTurnStateScanModel}
		observed, observedErr := s.repo.ListObservedOpenAICodexTurnStateModels(ctx, account.ID)
		if observedErr == nil {
...
func (s *openAICodexTurnStateScanner) runJob(ctx context.Context, job openAICodexTurnStateScanJob) {
	...
	if err != nil || account == nil || account.ParentAccountID != nil || !account.IsOpenAIOAuthLike() {
		return
	}
...
func (s *OpsService) ListOpenAICodexTurnStateAccountStatuses(ctx context.Context, accountIDs []int64, page, pageSize int) (*OpenAICodexTurnStateAccountStatusList, error) {
	...
	result, err := repo.ListOpenAICodexTurnStateAccountStatuses(ctx, accountIDs, page, pageSize)
	if err != nil {
		return nil, err
	}
...
func (s *OpsService) GetOpenAICodexTurnStateOperationsSummary(ctx context.Context) (*OpenAICodexTurnStateOperationsSummary, error) {
	...
	return repo.GetOpenAICodexTurnStateOperationsSummary(ctx)
}
...
func (s *OpsService) EnqueueAllOpenAICodexTurnStateScans(ctx context.Context) (int, error) {
	...
	for i := range accounts {
		...
		models := []string{defaultOpenAICodexTurnStateScanModel}
		observed, observedErr := s.codexTurnStateScanner.repo.ListObservedOpenAICodexTurnStateModels(ctx, account.ID)
		if observedErr == nil {
			models = append(models, observed...)
		}
		seen := make(map[string]struct{}, len(models))
		for _, model := range models {
			...
			seen[model] = struct{}{}
			if s.codexTurnStateScanner.Enqueue(account.ID, model, true) {
				queued++
			}
		}
	}
