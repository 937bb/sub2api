...
func TestHarvestOpenAICodexTicket_StopsAt292AndUsesHarvestProxy(t *testing.T) {
	...
	harvestVersion := upstream.requests[0].Header.Get("version")
	require.GreaterOrEqual(t, CompareVersions(harvestVersion, openAICodexAstraMinVersion), 0)
	require.Equal(t, buildCodexCLIUserAgent(harvestVersion), upstream.requests[0].Header.Get("user-agent"))
