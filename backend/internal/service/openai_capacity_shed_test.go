...
func TestOpenAIStreamMetadataPreambleAndMessageOnlyOverloadFailOver(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			require.Truef(t, failoverErr.RetryableOnSameAccount,
				"unexpected failover classification: status=%d scoped=%t body=%s",
				failoverErr.StatusCode, failoverErr.RequestScopedTransient, failoverErr.ResponseBody)
			require.True(t, failoverErr.RequestScopedTransient)
