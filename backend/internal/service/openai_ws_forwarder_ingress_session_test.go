...
func TestOpenAIGatewayService_ProxyResponsesWebSocketFromClient_PassthroughHeadersUsePromptCacheAndTurnState(t *testing.T) {
	...
	require.Equal(t, fingerprintIDs.sessionID, captureDialer.lastHeaders.Get("session_id"))
	...
	require.Equal(t, fingerprintIDs.sessionID, gjson.Get(forwarded, "client_metadata.session_id").String())
	require.Equal(t, fingerprintIDs.installationID, captureDialer.lastHeaders.Get("x-codex-installation-id"))
	require.Equal(t, fingerprintIDs.installationID, gjson.Get(forwarded, "client_metadata.x-codex-installation-id").String())
	...
	require.Equal(t, gjson.Get(forwarded, "client_metadata.turn_id").String(), gjson.Get(turnMetadata, "turn_id").String())
