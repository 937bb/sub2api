...
func applyCodexImageGenerationBridgeInstructions(reqBody map[string]any) bool {
	...
	if strings.Contains(existing, codexImageGenerationBridgeMarker) || strings.Contains(existing, legacyCodexImageGenerationBridgeMarker) {
		return false
	}
...
func applyCodexSparkImageUnsupportedInstructions(reqBody map[string]any) bool {
	...
	existing, _ := reqBody["instructions"].(string)
	if strings.Contains(existing, codexSparkImageUnsupportedMarker) || strings.Contains(existing, legacyCodexSparkImageUnsupportedMarker) {
		return false
	}
	existing = strings.TrimRight(existing, " \t\r\n")
	if strings.TrimSpace(existing) == "" {
...
func filterCodexInputWithOptions(input []any, opts codexInputFilterOptions) []any {
	...
	for _, item := range input {
		...
		if typ == "reasoning" {
			newItem := make(map[string]any, len(m))
			for key, value := range m {
				if key == "id" || key == "call_id" || key == "status" {
					// Strip replay-only identity and lifecycle metadata.
					continue
				}
				newItem[key] = value
			}
...
// isCodexToolCallInputType matches call-input types (not outputs). Individual
// item ID prefix contracts are handled by shouldStripOpenAIResponsesInputItemID;
// custom_tool_call requires ctc rather than fc.
...
func isCodexFunctionCallInputType(typ string) bool {
	switch typ {
	case "function_call",
		"tool_call",
		"local_shell_call",
