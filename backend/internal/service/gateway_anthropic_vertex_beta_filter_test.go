...
func newVertexBetaTestServiceAccount(id int64) *Account {
	return &Account{
		ID:       id,
...
func TestVertexBetaFilter_StripsUnsupportedClaudeCodeTokens(t *testing.T) {
	...
	req, _, err := svc.buildUpstreamRequest(
		context.Background(), c, newVertexBetaTestServiceAccount(401), body,
	...
	for _, token := range []string{
		...
		require.False(t, anthropicBetaTokensContains(outBeta, token),
			"token %q should be stripped from Vertex outgoing beta %q", token, outBeta)
	}
	for _, token := range []string{
		...
		require.True(t, anthropicBetaTokensContains(outBeta, token),
			"token %q should be preserved in Vertex outgoing beta %q", token, outBeta)
	}
}
...
func TestVertexBetaFilter_DropsHeaderWhenAllUnsupported(t *testing.T) {
	...
	svc := &GatewayService{}
	req, _, err := svc.buildUpstreamRequest(
		context.Background(), c, newVertexBetaTestServiceAccount(402), body,
		"vertex-token", "service_account", "claude-opus-4-7@20260417", false, false,
	)
	require.NoError(t, err)
...
func TestVertexBetaFilter_BodySanitizeKeysOnFinalBeta(t *testing.T) {
	...
	svc := &GatewayService{}
	req, _, err := svc.buildUpstreamRequest(
		context.Background(), c, newVertexBetaTestServiceAccount(403), body,
		"vertex-token", "service_account", "claude-opus-4-7@20260417", false, false,
	)
	require.NoError(t, err)
...
func TestVertexBetaFilter_BlocksViaBetaPolicy(t *testing.T) {
	...
	_, _, err = svc.buildUpstreamRequest(
		context.Background(), c, newVertexBetaTestServiceAccount(404), body,
		"vertex-token", "service_account", "claude-opus-4-7@20260417", false, false,
	)
	require.Error(t, err)
