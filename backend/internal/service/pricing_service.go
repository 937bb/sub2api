...
// normalizeGeminiThinkingTierAlias maps Antigravity's Gemini Flash thinking-tier
// model IDs to their public base model. The tier controls reasoning behavior,
// not the published token rate, so tiered requests use the base price card.
func normalizeGeminiThinkingTierAlias(model string) string {
	for _, baseModel := range []string{"gemini-3.6-flash", "gemini-3.8-flash"} {
