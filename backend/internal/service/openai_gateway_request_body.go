...
// Codex advertises "none" for visible non-reasoning models, while compatible
// Responses providers often reject that placeholder. Preserve it for the
// official OpenAI endpoint and strip it for compatible API-key endpoints.
...
// while OpenAI accepts only the portable reasoning fields on replay.
...
func forceOpenAIPriorityTierInBody(apiKey *APIKey, account *Account, body []byte) ([]byte, error) {
	if !shouldForceOpenAIPriorityTier(apiKey) || account == nil || account.Platform != PlatformOpenAI || len(body) == 0 {
		return body, nil
	}
	if gjson.GetBytes(body, "service_tier").String() == OpenAIFastTierPriority {
		return body, nil
	}
...
// group policy rewriting and model-family remapping (max -> xhigh).
...
// CanonicalRequestedReasoningEffortFromReqBody preserves the explicit effort
// from an unmodified request body before group policy or model remapping.
