...
func TestOpenAIGatewayService_Forward_WSv2_ExecutionScopeUsesOriginalIdentity(t *testing.T) {
	...
	completed := []byte(`{"type":"response.completed","response":{"id":"resp_exec_scope","model":"gpt-5.1","usage":{"input_tokens":1,"output_tokens":1}}}`)
	...
	pool.setClientDialerForTest(&openAIWSCaptureDialer{
		...
		connFactory: func() *openAIWSCaptureConn {
			return &openAIWSCaptureConn{events: [][]byte{completed}}
		},
	})
	defer pool.Close()
