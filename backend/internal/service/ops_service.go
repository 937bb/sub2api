	...
	if openAIGatewayService != nil {
		...
		if scannerRepo, ok := opsRepo.(OpenAICodexTurnStateScannerRepository); ok {
			...
			if svc.codexTurnStateScanner != nil {
				openAIGatewayService.setOpenAICodexTurnStateScanEnqueuer(func(accountID int64, model string, force bool) bool {
					return svc.codexTurnStateScanner.Enqueue(accountID, model, force)
				})
			}
		}
	}
