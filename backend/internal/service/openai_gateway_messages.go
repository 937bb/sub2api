...
func (s *OpenAIGatewayService) ForwardAsAnthropic(
	...
	forcedBody, forceErr := forceOpenAIPriorityTierInBody(getAPIKeyFromContext(c), account, responsesBody)
	if forceErr != nil {
		return nil, forceErr
	}
	...
	if account.Platform != PlatformGrok && promptCacheKey != "" && !account.IsOpenAIOAuthLike() {
	...
	if account.UsesOpenAICodexProtocol() && account.Platform != PlatformGrok {
		...
		enforceCodexIdentityHeadersWithUA(upstreamReq.Header, s.codexIdentityOverrideUA(codexAccountIdentitySource(c, account)))
