...
func (s *RateLimitService) handle429(ctx context.Context, account *Account, headers http.Header, responseBody []byte) {
	...
	if account.Platform == PlatformOpenAI {
		...
		if account.IsOpenAIOAuthLike() && s.IsOpenAIOAuth429DynamicEnabled(ctx, account) {
			s.RecordOpenAIOAuthUpstreamOutcome(ctx, account, http.StatusTooManyRequests, ParseCodexRateLimitHeaders(headers))
			return
		}
...
func calculateOpenAI429ResetTime(headers http.Header) *time.Time {
	...
	// A reset-after header describes the window, not exhausted quota, when
	// usage is below 100%. Let transient 429s use the configured fallback.
	return nil
}
