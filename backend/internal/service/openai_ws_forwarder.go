...
type OpenAIWSIngressHooks struct {
	...
	// ResolveRequestModel also reports whether the mapped model is a final
	// group-level upstream ID that must skip account mapping and aliasing.
	ResolveRequestModel func(turn int, originalModel string) (mappedModel string, final bool, err error)
...
func (s *OpenAIGatewayService) getOpenAIWSPassthroughDialer() openAIWSClientDialer {
	...
	s.openaiWSPassthroughDialerOnce.Do(func() {
		if s.openaiWSPassthroughDialer == nil {
			s.openaiWSPassthroughDialer = newConfiguredOpenAIWSClientDialer(s.cfg, ResolveOpenAIWSUpstreamReadLimitBytes(s.cfg))
		}
	})
