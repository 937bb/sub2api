...
func TestOpenAIGatewayService_Forward_HTTPIngressUsesWSWhenBridgeEnabled(t *testing.T) {
...
func TestOpenAIGatewayService_Forward_HTTPIngressStaysHTTPWhenBridgeDisabled(t *testing.T) {
	...
	cfg.Gateway.OpenAIWS.APIKeyEnabled = true
	cfg.Gateway.OpenAIWS.ResponsesWebsocketsV2 = true
	cfg.Gateway.OpenAIWS.HTTPIngressUpstreamWSEnabled = false
...
func TestOpenAIGatewayService_Forward_WSv2Dial401FallbackHTTP(t *testing.T) {
	gin.SetMode(gin.TestMode)
	var wsAttempts atomic.Int32
	ws401Server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		wsAttempts.Add(1)
		w.WriteHeader(http.StatusUnauthorized)
		_, _ = w.Write([]byte(`unauthorized`))
	}))
	defer ws401Server.Close()
	...
	upstream := &httpUpstreamRecorder{
		resp: &http.Response{
			...
			Header:     http.Header{"Content-Type": []string{"application/json"}},
			Body: io.NopCloser(strings.NewReader(
				`{"id":"resp_http_after_ws_401","usage":{"input_tokens":8,"output_tokens":9,"input_tokens_details":{"cached_tokens":1}}}`,
			)),
		},
	}
	...
	require.False(t, result.OpenAIWSMode)
	require.NotNil(t, upstream.lastReq)
	require.Equal(t, int32(1), wsAttempts.Load(), "handshake 401 must not be retried over WS")
	require.Equal(t, http.StatusOK, rec.Code)
	require.True(t, svc.isOpenAIWSFallbackCooling(account.ID))
	require.Equal(t, "handshake_unauthorized", c.GetString("openai_ws_fallback_reason"))
}
