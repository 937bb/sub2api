...
// effectiveSameAccountRetryLimit combines the account pool budget with an
// explicit request-scoped override (for example, a quota-bypass half-open
// probe), then applies the service-provided cap.
func effectiveSameAccountRetryLimit(failoverErr *service.UpstreamFailoverError, account *service.Account) int {
	...
	if account.IsPoolMode() {
		fallback = account.GetPoolModeRetryCount()
	}
	limit := fallback
	if failoverErr != nil {
		limit = failoverErr.ResolveSameAccountRetryLimit(fallback)
		if limit > 0 && failoverErr.SameAccountRetryMax > 0 && failoverErr.SameAccountRetryMax < limit {
