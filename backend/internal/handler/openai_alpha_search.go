...
func (h *OpenAIGatewayHandler) AlphaSearch(c *gin.Context) {
	...
	channelMapping := h.resolveOpenAIRequestModelMapping(c, apiKey, requestedModel)
	...
	for {
		...
		if err == nil {
			h.gatewayService.ReportOpenAIAccountScheduleResult(account.ID, account.GetMappedModel(requestedModel), true, nil, account)
			if result != nil {
