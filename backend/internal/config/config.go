...
type GatewayOpenAIWSConfig struct {
	...
	// UpstreamReadLimitBytes: 上游 WS 单消息解压后的读取上限。
	UpstreamReadLimitBytes int64 `mapstructure:"upstream_read_limit_bytes"`
...
func setDefaults() {
	...
	viper.SetDefault("gateway.openai_ws.upstream_read_limit_bytes", 64*1024*1024)
	...
	viper.SetDefault("gateway.openai_ws.dial_timeout_seconds", 8)
...
func (c *Config) Validate() error {
	...
	if c.Gateway.OpenAIWS.UpstreamReadLimitBytes <= 0 {
		return fmt.Errorf("gateway.openai_ws.upstream_read_limit_bytes must be positive")
	}
	if c.Gateway.OpenAIWS.UpstreamReadLimitBytes > 256*1024*1024 {
		return fmt.Errorf("gateway.openai_ws.upstream_read_limit_bytes must not exceed 256 MiB")
	}
	...
	if c.Gateway.OpenAIScheduler.QuotaBypassSoftConcurrency < 0 {
		return fmt.Errorf("gateway.openai_scheduler.quota_bypass_soft_concurrency must be non-negative")
	}
