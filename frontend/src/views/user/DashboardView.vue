...
const loadStats = async () => { loading.value = true; try { const [dashboardStats] = await Promise.all([usageAPI.getDashboardStats(), authStore.refreshUser().catch((error) => { console.error('Failed to refresh dashboard user:', error) })]); stats.value = dashboardStats } catch (error) { console.error('Failed to load dashboard stats:', error) } finally { loading.value = false } }
