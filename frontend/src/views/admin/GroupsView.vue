  ...
  <AppLayout>
    ...
    >
      ...
      >
        ...
        <!-- Pinned Codex manifest accounts are editable only for OpenAI groups. -->
...
const handleEdit = async (group: AdminGroup) => {
  ...
  // Resolve saved manifest account names asynchronously, falling back to IDs.
  ...
  for (const id of editCodexManifestConfig.value.account_ids) {
    adminAPI.accounts
      ...
      .catch(() => {
        // The component falls back to #<id> when a stored account cannot be resolved.
      });
  }
  // 加载模型路由规则（异步加载账号名称）
