        ...
        <section v-for="group in filteredGroups" :key="group.id" class="overflow-hidden rounded-md border border-gray-200 bg-white dark:border-dark-700 dark:bg-dark-900">
          ...
          <div v-if="group.rules.length" class="overflow-x-auto">
            <table class="w-full min-w-[900px] table-fixed text-left text-sm">
              <colgroup><col class="w-[90px]" /><col class="w-[180px]" /><col class="w-[180px]" /><col class="w-[180px]" /><col class="w-[150px]" /><col class="w-[120px]" /><col class="w-[72px]" /></colgroup>
              ...
              <tbody>
                <tr v-for="rule in group.rules" :key="rule.id" class="border-t border-gray-100 dark:border-dark-800">
                  ...
                  <td class="px-4 py-3"><input v-model="rule.start" class="input" :class="ruleClass(group, rule)" type="time" /></td>
                  <td class="px-4 py-3"><input v-model="rule.end" class="input" :class="ruleClass(group, rule)" type="time" /></td>
                  ...
                  <td class="px-4 py-3 text-center">
                    <span v-if="isCrossDay(rule)" class="badge badge-warning">{{ t('timeBilling.crossDay') }}</span>
                    <span v-else class="text-xs text-gray-400 dark:text-dark-500">{{ t('timeBilling.sameDay') }}</span>
                  </td>
...
import type { AdminGroup, GroupPlatform, TimeBillingRule } from '@/types'
...
const filteredGroups = computed(() => groups.value.filter((group) => {
  ...
  if (ruleStateFilter.value === 'cross_day' && !group.rules.some((rule) => rule.enabled && isCrossDay(rule))) return false
  const query = search.value.toLocaleLowerCase()
  if (!query) return true
  const ruleText = group.rules.map((rule) => `${rule.start} ${rule.end} ${rule.rate_multiplier}`).join(' ')
...
function cloneRule(rule: TimeBillingRule): TimeBillingRule {
  return { id: rule.id, enabled: Boolean(rule.enabled), start: normalizeTime(rule.start), end: normalizeTime(rule.end), rate_multiplier: Number(rule.rate_multiplier ?? 1) }
}
function groupFromAPI(group: AdminGroup): TimeBillingGroupRow {
  ...
  if (!rules.length && group.peak_rate_enabled && group.peak_start && group.peak_end) {
    rules = [{ id: `legacy-${group.id}`, enabled: true, start: normalizeTime(group.peak_start), end: normalizeTime(group.peak_end), rate_multiplier: Number(group.peak_rate_multiplier ?? 1) }]
  }
...
function ruleRanges(rule: TimeBillingRule): MinuteRange[] {
  const start = parseMinute(rule.start); const end = parseMinute(rule.end)
  if (start === null || end === null || start === end) return []
  if (start < end) return [{ start, end }]
  return end > 0 ? [{ start, end: 1440 }, { start: 0, end }] : [{ start, end: 1440 }]
}
function rulesOverlap(left: TimeBillingRule, right: TimeBillingRule): boolean {
...
function isRuleValid(rule: TimeBillingRule): boolean {
...
function ruleClass(group: TimeBillingGroupRow, rule: TimeBillingRule): string {
  return !isRuleValid(rule) || overlappingIDs(group).has(rule.id) ? 'border-red-400 focus:border-red-500 focus:ring-red-500' : ''
}
function isCrossDay(rule: TimeBillingRule): boolean {
  const start = parseMinute(rule.start); const end = parseMinute(rule.end)
  return start !== null && end !== null && start > end
}
function formatMultiplier(value: number): string { return Number(value || 0).toFixed(3) }
function effectiveMultiplier(group: TimeBillingGroupRow, rule: TimeBillingRule): string { return formatMultiplier(group.baseMultiplier * Number(rule.rate_multiplier || 0)) }
...
function addRule(group: TimeBillingGroupRow): void {
  group.rules.push({ id: crypto.randomUUID(), enabled: false, start: '09:00', end: '18:00', rate_multiplier: 1 })
}
...
async function saveGroup(group: TimeBillingGroupRow): Promise<void> {
  ...
  try {
    const updated = await adminAPI.groups.update(group.id, { time_billing_rules: group.rules.map(cloneRule) })
