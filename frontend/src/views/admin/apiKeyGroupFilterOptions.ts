...
// Section-header sentinels are negative so they never collide with real group IDs.
...
export function buildApiKeyGroupFilterOptions(
  ...
  for (const [label, value, items] of sections) {
    if (items.length === 0) continue
    options.push({ value, label, kind: 'group', disabled: true })
    options.push(...items)
  }
