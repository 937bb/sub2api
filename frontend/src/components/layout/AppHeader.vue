  ...
  <header class="glass sticky top-0 z-30 border-b border-gray-200/50 dark:border-dark-700/50">
    <div class="flex h-16 items-center justify-between px-4 md:px-6">
      ...
      <div class="flex items-center gap-3">
        ...
        <button
          v-if="user && checkinVisible"
          @click="doQuickCheckin"
          :disabled="checkinBusy || checkinDone"
          class="hidden items-center gap-1.5 rounded-lg px-2.5 py-1.5 text-sm font-medium transition-colors sm:flex"
          :class="checkinDone
            ? 'bg-green-50 text-green-600 dark:bg-green-900/20 dark:text-green-400 cursor-default'
            : 'text-amber-600 hover:bg-amber-50 dark:text-amber-400 dark:hover:bg-amber-900/20'"
          :title="checkinDone ? t('checkin.alreadyCheckedIn') : t('checkin.button')"
        >
          <svg v-if="checkinBusy" class="h-4 w-4 animate-spin" fill="none" viewBox="0 0 24 24">
            <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4" />
            <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z" />
          </svg>
          <svg v-else-if="checkinDone" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
          ...
          <span class="hidden md:inline">{{ checkinDone ? t('checkin.alreadyCheckedIn') : t('checkin.button') }}</span>
        </button>
...
async function doQuickCheckin() {
  if (checkinBusy.value || checkinDone.value) return
