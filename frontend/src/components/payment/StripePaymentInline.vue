          ...
          <div class="w-full rounded-xl bg-gray-50 p-4 dark:bg-dark-800">
            <div class="space-y-2 text-sm">
              ...
              <div v-if="amount > 0" class="flex justify-between">
                ...
                <span class="font-medium text-gray-900 dark:text-white">{{ formatProductAmount(amount) }}</span>
              </div>
              <div class="flex justify-between">
...
function formatGatewayAmount(amount: number): string {
  return formatPaymentAmount(amount, props.currency, localeCode.value)
}

function formatProductAmount(amount: number): string {
  const currency = props.orderType === 'balance' ? 'USD' : props.currency
