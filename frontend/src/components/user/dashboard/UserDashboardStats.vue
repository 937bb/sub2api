<template>
  ...
  <div class="grid grid-cols-2 gap-4 lg:grid-cols-4">
    ...
    <div v-if="!isSimple" class="card p-5">
      <div class="flex items-start justify-between">
        <span class="text-sm font-medium text-gray-500 dark:text-gray-400">{{ t('dashboard.balance') }}</span>
      ...
      <p class="font-display mt-3 text-3xl font-bold tracking-tight text-emerald-600 dark:text-emerald-400">{{ balanceLabel }}</p>
      <p class="mt-2 text-xs text-gray-500 dark:text-gray-400">{{ t('common.available') }}</p>
    </div>

    <!-- API Keys -->
    <div class="card p-5">
      <div class="flex items-start justify-between">
        <span class="text-sm font-medium text-gray-500 dark:text-gray-400">{{ t('dashboard.apiKeys') }}</span>
        <div class="rounded-xl bg-blue-100 p-2 dark:bg-blue-900/30">
          <Icon name="key" size="md" class="text-blue-600 dark:text-blue-400" :stroke-width="2" />
        </div>
      </div>
      <p class="font-display mt-3 text-3xl font-bold tracking-tight text-gray-900 dark:text-white">{{ stats?.total_api_keys || 0 }}</p>
      <span class="mt-2 inline-flex rounded-full bg-green-100 px-2 py-0.5 text-xs font-medium text-green-700 dark:bg-green-900/30 dark:text-green-400">{{ stats?.active_api_keys || 0 }} {{ t('common.active') }}</span>
    </div>

    <!-- Today Requests -->
    <div class="card p-5">
      <div class="flex items-start justify-between">
        <span class="text-sm font-medium text-gray-500 dark:text-gray-400">{{ t('dashboard.todayRequests') }}</span>
        <div class="rounded-xl bg-green-100 p-2 dark:bg-green-900/30">
          <Icon name="chart" size="md" class="text-green-600 dark:text-green-400" :stroke-width="2" />
        </div>
      </div>
      <p class="font-display mt-3 text-3xl font-bold tracking-tight text-gray-900 dark:text-white">{{ stats?.today_requests || 0 }}</p>
      <p class="mt-2 text-xs text-gray-500 dark:text-gray-400">{{ t('common.total') }}: {{ formatNumber(stats?.total_requests || 0) }}</p>
    </div>

    <!-- Today Cost -->
    <div class="card p-5">
      <div class="flex items-start justify-between">
        <span class="text-sm font-medium text-gray-500 dark:text-gray-400">{{ t('dashboard.todayCost') }}</span>
      ...
      <p class="font-display mt-3 text-3xl font-bold tracking-tight text-gray-900 dark:text-white">
        ...
        <span class="text-base font-normal text-gray-400 dark:text-gray-500" :title="t('dashboard.standard')"> / ${{ formatCost(stats?.today_cost || 0) }}</span>
      </p>
  ...
  <div class="card grid grid-cols-2 divide-y divide-gray-200/70 dark:divide-dark-700 sm:grid-cols-4 sm:divide-x sm:divide-y-0">
    <!-- Today Tokens -->
    <div class="p-5">
      <div class="flex items-center gap-2">
        <Icon name="cube" size="sm" class="text-amber-500 dark:text-amber-400" :stroke-width="2" />
        <span class="text-xs font-medium text-gray-500 dark:text-gray-400">{{ t('dashboard.todayTokens') }}</span>
      </div>
      <p class="font-display mt-2 text-2xl font-bold tracking-tight text-gray-900 dark:text-white">{{ formatTokens(stats?.today_tokens || 0) }}</p>
      <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">{{ t('dashboard.input') }}: {{ formatTokens(stats?.today_input_tokens || 0) }} / {{ t('dashboard.output') }}: {{ formatTokens(stats?.today_output_tokens || 0) }}</p>
    </div>
    ...
    <div class="p-5">
      <div class="flex items-center gap-2">
        ...
        <span class="text-xs font-medium text-gray-500 dark:text-gray-400">{{ t('dashboard.totalTokens') }}</span>
      </div>
      <p class="font-display mt-2 text-2xl font-bold tracking-tight text-gray-900 dark:text-white">{{ formatTokens(stats?.total_tokens || 0) }}</p>
      <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">{{ t('dashboard.input') }}: {{ formatTokens(stats?.total_input_tokens || 0) }} / {{ t('dashboard.output') }}: {{ formatTokens(stats?.total_output_tokens || 0) }}</p>
    </div>
    ...
    <div class="p-5">
      <div class="flex items-center gap-2">
        ...
        <span class="text-xs font-medium text-gray-500 dark:text-gray-400">{{ t('dashboard.performance') }}</span>
      </div>
      <div class="mt-2 flex items-baseline gap-2">
        <p class="font-display text-2xl font-bold tracking-tight text-gray-900 dark:text-white">{{ formatTokens(stats?.rpm || 0) }}</p>
    ...
    <div class="p-5">
      <div class="flex items-center gap-2">
        ...
        <span class="text-xs font-medium text-gray-500 dark:text-gray-400">{{ t('dashboard.avgResponse') }}</span>
      </div>
      <p class="font-display mt-2 text-2xl font-bold tracking-tight text-gray-900 dark:text-white">{{ formatDuration(stats?.average_duration_ms || 0) }}</p>
