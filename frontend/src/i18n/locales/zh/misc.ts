export default {
  ...
  userSubscriptions: {
    ...
    advanceDailyQuota: '提前恢复额度',
    advanceDailyQuotaTitle: '确认提前恢复日额度',
    advanceDailyQuotaConfirm:
      '请核对下方额度和到期时间。确认后将立即扣除 24 小时有效期，此操作不可撤销。',
    advanceDailyQuotaBlocked: '当前不满足提前恢复条件，不会扣除订阅时间。',
    advancePreviewLoadingMessage: '正在核对最新额度和到期时间。',
    loadingAdvancePreview: '正在加载恢复明细…',
    advancePreviewFailed: '加载恢复明细失败',
    advanceUnavailable: '暂时无法提前恢复',
    dailyQuota: '完整日额度',
    naturalDailyReset: '等待自然恢复',
    weeklyRemaining: '周剩余额度',
    monthlyRemaining: '月剩余额度',
    recoverableQuota: '本次实际可恢复',
    validityDeduction: '扣除有效期',
    currentExpiry: '当前到期时间',
    expiryAfterAdvance: '扣除后到期时间',
    hours: '{hours} 小时',
    resetAtDetail: '{time} 后恢复（{date}）',
    noResetBeforeExpiry: '订阅到期前不再恢复',
    availableNow: '已到恢复时间',
    periodUsagePreservedWarning: '周用量和月用量不会清零；本次只恢复一份完整日额度。',
    advanceBlockers: {
      subscription_inactive: '订阅已过期、暂停或不可用',
      one_time_subscription: '24 小时日卡只提供一次额度，不能提前恢复',
      insufficient_term: '订阅剩余有效期不足 24 小时',
      daily_quota_not_configured: '当前套餐没有配置日额度',
      daily_quota_already_reset: '日额度已到自然恢复时间，无需扣除有效期',
      daily_quota_not_exhausted: '当前日额度尚未用完',
      weekly_quota_insufficient: '周剩余额度不足一份完整日额度',
      monthly_quota_insufficient: '月剩余额度不足一份完整日额度',
      unknown: '额度状态已变化，请刷新后重试'
    },
    advanceDailyQuotaSuccess: '日额度已恢复，订阅有效期已扣除 24 小时',
    advanceDailyQuotaFailed: '恢复日额度失败',
    advancingDailyQuota: '处理中'
  },
