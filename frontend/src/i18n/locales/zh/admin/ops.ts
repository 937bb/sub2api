    ...
    ops: {
      ...
      turnState: {
        ...
        metrics: {
          ...
          readyAccounts: '全部模型就绪',
          missingAccounts: '未完整账号',
          readyModelSlots: '模型 State 就绪',
          runningJobs: '扫描任务',
          enabledProxies: '启用代理',
          healthyProxies: '健康代理',
          sharedProxies: '复用代理'
        ...
        scanSettings: {
          ...
          lengthsHint: '严格白名单，从左到右优先，使用逗号分隔，例如 332, 292。其他长度会被拒绝，不会复用。',
          rules: '套餐 / 模型规则',
          rulesHint: '优先级：套餐 + 模型 > 套餐 + 全部模型 > 全部套餐 + 模型 > 全部套餐 + 全部模型 > 默认长度。模型填写 * 表示全部；长度从左到右优先。删除规则后按下一级规则生效。',
          selectedScope: '正在修改 {plan} / {model}：保存后适用于相同套餐和模型的所有账号，不是单账号设置；State 内容不会跨账号共享。',
          noRules: '暂无单独规则，使用默认目标长度。',
          addRule: '+ 添加规则',
          deleteRule: '删除第 {index} 条规则',
          plan: '订阅套餐',
          ruleLengths: '目标长度',
          invalidRuleModel: '请选择有效套餐并填写模型名称（最多 200 字符，无空格），或填写 * 表示全部模型。',
          invalidRuleLengths: '规则长度需填写 1–16 个不重复的整数，每个长度为 64–4096。',
          duplicateRule: '同一套餐与模型只能配置一条规则，请合并重复项。',
          tooManyRules: '最多配置 128 条套餐 / 模型规则。',
          planSwitches: '按套餐启用 State 扫描',
          ...
          routeBinding: 'State 扫描出口绑定',
          routeBindingHint: '开启后，只有带扫描出口票据的 State 才算就绪；业务请求会复用该账号、该模型获取 State 时使用的 IPv6 或绑定代理。旧的无绑定 State 会自动重新扫描。',
          routeBindingEnabled: '出口绑定已开启：State、Session 与扫描出口作为同一票据复用。',
        ...
        scanNow: '立即扫描',
        editTarget: '修改目标',
        targetLengths: '目标 {lengths}',
        scanAccount: '扫描该账号的全部目标模型',
        readyProgress: '{ready}/{total} 个模型就绪',
        failedCount: '{count} 个异常',
        noTargetModels: '暂无目标模型状态',
        compact: { scanning: '扫描中', retry: '重试', failed: '失败', expiring: '将过期', missing: '缺失' },
        ...
        deleteProxyConfirm: '确定删除代理“{name}”？',
