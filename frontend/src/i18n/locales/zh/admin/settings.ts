export default {
    settings: {
      ...
      gatewayForwarding: {
        ...
        codexTicketEnabled: 'Codex State 采集',
        codexTicketEnabledDesc:
          '关闭后不打票、不注入 x-codex-turn-state，按原链路转发。开启后后台打票，并在业务请求中覆盖该头。',
        codexTicketHarvestProxy: 'State 采集代理',
      ...
      openAIOAuth429Dynamic: {
        title: 'OpenAI 429 动态调度',
        description: '对 Codex 订阅账号的 429 进行窗口采样，达到阈值后才暂停账号调度',
        enabled: '启用动态 429 探测',
        enabledHint: '允许零星 429 后继续尝试；持续 429 达到阈值时自动短暂冷却',
        windowSeconds: '统计窗口（秒）',
        minSamples: '最少请求样本',
          min429: '最少 429 次数',
          ratioThreshold: '429 比例阈值（0-1）',
          blockSeconds: '触发后冷却（秒）',
          usageWindowCheck: '要求用量窗口耗尽',
          usageWindowCheckHint: '仅当上游报告的 5 小时或 7 天用量达到设定阈值时触发冷却',
          usage5hThreshold: '5 小时用量阈值（%）',
          usage7dThreshold: '7 天用量阈值（%）',
          missingDataFallback: '缺失用量回退等待（秒）',
          planOverrides: '套餐专用策略',
          planOverridesHint: '精确匹配 plan_type 后覆盖默认策略，即使默认策略关闭也可单独启用',
          planType: '套餐类型',
          addOverride: '添加策略',
          hint: '适用于 OAuth、Plus/Team、PAT 和 Setup Token；API Key 不受此配置影响。',
        saved: 'OpenAI 429 动态调度设置已保存',
        saveFailed: '保存 OpenAI 429 动态调度设置失败'
      },
      streamTimeout: {
        title: '流超时处理',
