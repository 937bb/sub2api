...
- `2026-09-15` release `0.2.4.10`: Override req/v3's Chinese language default on privacy/subscription/quota requests; synchronize image-generation OAuth/PAT identity; handle escaped XML and desktop AGENTS environment envelopes without rewriting quoted examples; align PAT delegation and bounded tool metadata headers. See [changes, validation and rollout boundaries](deploy/CODEX-OUTBOUND-FOLLOWUP-20260915.md).

- `2026-09-15` source release `0.2.4.9`: Normalize the `<timezone>` element in complete Codex `<environment_context>` text fragments, including requests with no `client_metadata`. Apply the existing Pacific policy to Responses HTTP, compact, converted chat/messages, and WS preparation while retaining user prose, paths, images, tool data and API-key boundaries. Replace proxy-branded client identity values through the paired Codex identity resolver instead of relabeling the Sub2API version as a Codex version. The comparison is pinned to official Codex source `19286b88190b1b81c2506af97dfca6ab261ff72b`; see [source comparison and limits](deploy/CODEX-ENVIRONMENT-CONTEXT-20260915.md). No production rollout is performed by publishing this source update.

- `2026-09-15` source release `0.2.4.8`: Synchronize Pacific OAuth/PAT request metadata and conversation isolation with the other repository's heartbeat/retry fixes. Normalize recognized optional timezone/locale metadata, remove generated proxy labels from compatibility markers and model descriptions, and neutralize the proxy product token in outbound User-Agent values without adding a missing Messages-bridge originator. Preserve numeric precision through identity metadata rewrites. See [the outbound audit](deploy/CODEX-OUTBOUND-REVIEW-20260915.md) for boundaries and validation. This source release does not itself deploy or change the production HTTP-only setting.

- `2026-09-11` cluster release `0.2.4.2`: Merge `origin/937sub2b` through `eeff23e50` while retaining the cluster's mapped-retry, WS transport recovery, usage and routing fixes. Keep the upstream Lite bridge, outbound identity consistency, early capacity failover and large-request JSON optimizations. The release number follows the requested `.2` suffix; it does not discard code from the upstream `0.2.4.5` label.
- Fix repeated Cloudflare 524 disconnects during long WS reasoning waits: the HTTP Responses bridge sends SSE comment keepalives at the configured stream interval (default 10 seconds). Heartbeats do not create upstream requests, semantic output, first-token timing or usage. One WS read remains active at a time; cancellation, failed downstream writes and callback panics cancel and join the reader before the lease is released, and successful connections remain reusable.
- Heartbeat bytes do not consume or block pre-output retries, protocol fallback or account failover. Failed attempts remain private; final errors after the HTTP 200 heartbeat use a complete `response.failed` SSE event, including when mapped retry is disabled. Observed output, tool items and usage prevent replay for both streaming and non-streaming WS paths. Existing retry budgets and upstream read deadlines remain authoritative.
- Responses WS interruptions after output now report `upstream_websocket_stream_error` with a stable explanation instead of the generic `Upstream request failed`. Empty local HTTP 499 responses use their final cancellation marker and monitoring filter; explicit upstream errors retain their details.
- Validation: full default service tests pass (126.813s), handler tests pass (37.368s), and admin-handler tests pass (0.214s). Added regression coverage includes delayed WS output/first-token timing, hidden 502/503 recovery, account failover after heartbeat, WS recovery and HTTP fallback, final SSE errors with retry on/off, cancellation, failed writes, panic cleanup, non-streaming no-replay protection, and preservation of the existing compact keepalive during response-header snapshots. Diagnostic tests requiring live accounts are excluded from the default suite.

- `2026-09-10` WS transport and terminal outcome repair (`0.2.4.1`): Restore one fresh-connection WS recovery for pre-output transport failures within the configured OAuth mapped-retry budget, reserving HTTP fallback when needed. Once a request falls back for an account, its remaining retries stay HTTP; another account evaluates WS afresh. Normal connection pooling and continuation affinity remain intact. The existing single HTTP protocol fallback is retained when the retry budget is exhausted. Observed output, tool items, usage and cancellation prevent replay.
- Failed or cancelled OpenAI requests without measured usage no longer create ordinary zero-usage rows across Responses, Messages, Chat Completions and native WS handlers. Successful zero-usage responses, partial billed usage, non-token units and cyber-policy records keep their existing treatment. Final client cancellation is classified as 499 and respects the monitoring setting for ignoring cancellation; hidden retry errors are not misreported as recovered success. Historical usage is retained.
- Exhausted WS mapped errors match passthrough rules using their upstream semantic status, retaining custom messages and monitoring exclusions. A pool excluded solely by channel model restrictions returns a specific 404 `model_not_found` instead of a generic 503; mixed exclusions and temporary capacity shortages preserve their prior behavior. No channel association, pricing, retry setting, IPv6 relay, CF pool or cluster role is changed.
- Validation: full default `internal/service` and `internal/handler` tests pass (106s and 37s). Added regressions cover WS recovery/reuse, shared budgets and final fallback, next-account WS reset, output/usage no-replay guards, terminal cancellation through the real Ops middleware including ignore settings, zero/partial/non-token usage, passthrough rules and all three channel-restriction scheduling paths.

- `2026-09-10` WS pre-output retry repair: Intercept eligible OAuth WS `error` and `response.failed` events before HTTP Responses output reaches the client, using the existing mapped retry budget. Buffered lifecycle events from failed attempts are discarded; successful output, tool/output items, usage, cancellation, credential/rate-limit/request/policy errors and protocol recovery retain their no-replay or dedicated handling. Preserve observed non-streaming WS results through the forwarding layer so the retry owner cannot replay work already performed. Exhaustion disables further same-account retries while allowing account failover, and final errors retain the sanitized upstream message after passthrough-rule handling. Focused regressions cover recovery, strict retry counts, output/usage guards, settings, cancellation, passthrough status and special error classification. Version remains `0.2.4.1`.

- `2026-09-10` upstream error message repair: Preserve the upstream explanation when a WebSocket error must terminate an HTTP Responses stream. Emit a complete `response.failed` event and mark it delivered so the handler does not append a generic failure. Record the upstream attempt for Operations. The SSE log parser keeps a preceding specific error message when a later terminal frame contains only a generic fallback, while preserving explicit custom terminal messages and terminal status/type/code. Retry budgets and account failover policy are unchanged. Full default service and handler tests pass, including real WebSocket regressions for overload before/after output, missing messages, failed terminal writes, and SSE log capture.

- `0.2.4.1` cluster update: Merge `origin/937sub2b` at `33005daea` while retaining the OAuth retry recovery/failover fixes below. All default Go test packages pass, including rechecks of the schema loader with a writable test directory and repository tests without the temporary download proxy. The separate legacy `unit` service suite still has the pre-existing image-test compilation failures listed below. Production rollout must preserve both group model columns and synchronize them from the configuration read by the currently running release before introducing the new model-allowlist reader.

- `2026-09-10` OAuth retry regression fix: Keep WS previous-response and encrypted-content recovery able to send the repaired request when mapped retry is enabled, while disabling only generic nested reconnects. Credential errors, errors that prohibit same-account retry, explicit stop decisions and bounded 429 probes retain their existing scheduler policy. Exhausting the mapped budget disables additional same-account attempts but preserves healthy-account failover; discard staged error bytes, headers and temporary markers before handing control back to the scheduler. Stream credential errors retain their typed classification and account-health handling. Focused service and handler regressions pass; the broader `unit` build is still blocked by pre-existing missing image-test symbols (`SynthesizedFromModelText`, `shouldCoolOpenAIImagesToolForError`, `openAIImagesOAuthUnavailableDefaultCooldown`).

- `0.2.1.7` mapped-error retry repair: OAuth HTTP ingress uses a same-account full-forward retry budget, including errors raised after HTTP transport and pre-output SSE parsing. Error responses up to 64 KiB are staged before client delivery; successful output remains streaming. Eligible matching failures replay the unchanged canonical input. The inner HTTP retry loop and generic WS reconnect attempt are disabled while this budget owns the request; protocol-specific recovery/fallback and typed scheduler policy remain separate. Existing output, returned billable results, cyber-policy/local denials and cancellation prevent replay. Only temporary response markers are cleared; upstream error history and compatibility-recovery state remain. Logs identify `mapped_status` and skipped reasons. This does not guarantee upstream exactly-once execution or recover streams whose output was already delivered; native WebSocket ingress remains outside this HTTP-forward wrapper.

- `0.2.1.7` UI adjustment: Align OAuth upstream retry settings with adjacent settings using the shared card background, divided heading and padded body; preserve all retry and save behavior.
- `0.2.1.7` retry observability: Add an independent Operations retry-log card with paginated request-ID lookup and expandable event details. Structured `oauth_retry` events carry gateway request ID, account ID and retry-chain ID, never request bodies or credentials. Events distinguish actual retries, exhaustion, cancellation, transport errors, unreplayable bodies and received response headers. Received headers do not prove stream completion or billing success. Uses existing shared system-log storage/retention and its best-effort ingestion; counts are event rows, not unique requests. Filters cover time and platform, not groups. Retry scope and billing behavior are unchanged; system-mapped stream 502 errors are not newly retried by this update.

- `0.1.179.3`: Keep quota-bypass injection active for HTTP and every WebSocket `response.create` shape, including missing/null/empty `input`, while preventing Codex local-tool corruption by replacing the colliding synthetic `shell` history item with a per-request random non-client function name; leave non-response WebSocket frames untouched.
- `0.1.179.2`: Preserve Codex local-tool execution on quota-bypass accounts by leaving conversation-backed WebSocket `response.create` frames without `input` untouched, while retaining synthetic bypass for explicit HTTP/WS inputs and returning a realistic successful result for injected no-op shell turns instead of an empty tool output.
- `0.1.179.1`: Merge official `main` v0.1.179 while preserving concentrated account scheduling and WaitPlan affinity at hard concurrency, bounded/coalesced OAuth 429 handling, quota-bypass HTTP/WS preprocessing, deterministic upstream 400 passthrough, Codex role/image compatibility, replayed item ID/status sanitization, and converged HTTP/WS fingerprints; retain official retry deadlines, endpoint capabilities, and OAuth WebSocket controls.
...
- `0.1.200`: Unify request-level OpenAI quota-bypass classification across scheduling, HTTP, and WS: inject turns that omit `input`, recognize genuine `function_call_output` turns as native bypass with zero synthetic pairs, keep compaction requests on ordinary quota-health routing, and only allow WS 429 same-account retries when the current turn actually used bypass.
