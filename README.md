...
- `0.2.1.7` mapped-error retry repair: OAuth HTTP ingress now uses one same-account full-forward retry budget, including errors raised after HTTP transport and pre-output SSE parsing. Error responses up to 64 KiB are staged before client delivery; successful output remains streaming. Matching final statuses replay the unchanged canonical input, and exhausted typed failover errors stop further pool retry/switching. The inner HTTP retry loop and generic WS reconnect attempt are disabled while this budget owns the request (protocol-specific recovery/fallback remains separate). Existing output, billable results, cyber-policy/local denials and cancellation prevent replay. Only temporary response markers are cleared; upstream error history and compatibility-recovery state remain. Logs identify `mapped_status` and skipped reasons. This does not guarantee upstream exactly-once execution or recover streams whose output was already delivered; native WebSocket ingress remains outside this HTTP-forward wrapper.
